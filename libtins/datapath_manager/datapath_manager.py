...
def remove(args):
    ...
    print("Disassociating all stations")
    associated_station_set = set([])
    flows = ovs_controller.get_flows()
    for flow in flows:
        if ('in_port=' not in flow) or ('dl_src=' not in flow) or ('output:' not in flow):
            continue

        actions = flow[flow.find('actions=') + len('actions='):].split(',')
        actions = [x for x in actions if 'output:' in x][0]
        out_port = actions[actions.find('output:') + len('output:')]
        in_port = flow[flow.find('in_port=') + len('in_port=')]

        if in_port == str(ethernet_interface_index) and out_port == str(virtual_interface_index):
            src_index = flow.find('dl_src=')
            mac = flow[src_index + len('dl_src='): src_index + len('dl_src=') + len('xx:xx:xx:xx:xx:xx')]
            associated_station_set.add(mac)

    station_flow = {}
    station_flow['in_port'] = ethernet_interface_index
    for mac_addr in associated_station_set:
        print("Disassociating station %s" % mac_addr)
        station_flow['dl_src'] = mac_addr
        ovs_controller.remove_flow(station_flow)
